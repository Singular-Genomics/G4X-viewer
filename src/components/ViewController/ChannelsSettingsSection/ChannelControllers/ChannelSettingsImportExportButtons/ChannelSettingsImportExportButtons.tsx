...
export const ChannelSettingsImportExportButtons = () => {
  ...
  const [channelOptions] = useViewerStore(
    useShallow((store) => [store.channelOptions])
  );
    ...
    (acceptedFiles: File[]) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          const { addIsChannelLoading, setIsChannelLoading } =
              ...
              channelsToAdd.forEach((channelData: any) => {
                if (channelData.name) {
                  ...
                  if (channelIndex !== -1) {
                    ...
                    const numSelectionsBeforeAdd =
                      useChannelsStore.getState().selections.length;
